...
function rgb2hex$1(rgb) {
	const [R$1, G$1, B$1, A$1] = rgb;
	let value = "#";
	const newR = R$1.toString(16);
	const newG = G$1.toString(16);
	const newB = B$1.toString(16);
	value += newR.length > 1 ? newR : `0${newR}`;
	value += newG.length > 1 ? newG : `0${newG}`;
	value += newB.length > 1 ? newB : `0${newB}`;
	if (typeof A$1 !== "undefined" && rgb.length > 3) {
		const newA = Math.round(A$1 * 255).toString(16);
		value += newA.length > 1 ? newA : `0${newA}`;
	}
	return value;
}
function rgb2lab$1(rgb) {
	let r$1 = rgb[0] / 255, g = rgb[1] / 255, b = rgb[2] / 255, x$1, y$1, z$1;
	r$1 = r$1 > .04045 ? Math.pow((r$1 + .055) / 1.055, 2.4) : r$1 / 12.92;
	g = g > .04045 ? Math.pow((g + .055) / 1.055, 2.4) : g / 12.92;
	b = b > .04045 ? Math.pow((b + .055) / 1.055, 2.4) : b / 12.92;
	x$1 = (r$1 * .4124 + g * .3576 + b * .1805) / .95047;
	y$1 = (r$1 * .2126 + g * .7152 + b * .0722) / 1;
	z$1 = (r$1 * .0193 + g * .1192 + b * .9505) / 1.08883;
	x$1 = x$1 > .008856 ? Math.pow(x$1, .3333333333333333) : 7.787 * x$1 + .13793103448275862;
	y$1 = y$1 > .008856 ? Math.pow(y$1, .3333333333333333) : 7.787 * y$1 + .13793103448275862;
	z$1 = z$1 > .008856 ? Math.pow(z$1, .3333333333333333) : 7.787 * z$1 + .13793103448275862;
	return [
		116 * y$1 - 16,
		500 * (x$1 - y$1),
		200 * (y$1 - z$1)
...
var init_dist = __esm({ "../sdk/dist/index.js"() {
	(function(l$1, r$1) {
		if (!l$1 || l$1.getElementById("livereloadscript")) return;
		r$1 = l$1.createElement("script");
		r$1.async = 1;
		r$1.src = "//" + (self.location.host || "localhost").split(":")[0] + ":35729/livereload.js?snipver=1";
		r$1.id = "livereloadscript";
		l$1.getElementsByTagName("head")[0].appendChild(r$1);
	})(self.document);
	ImageColorUtils$1 = class ImageColorUtils$2 {
		...
		pickColor(x$1, y$1) {
			return ImageColorUtils$2.getRGB(this.imageData.data, x$1, y$1, this.canvas.width);
		}
		pickLineColor({ leftTopPosition, rightBottomPosition, scopes }) {
			...
			for (const key in lineArrayCollection) {
				...
				for (const position of lineArray) {
					const x$1 = position[0];
					const y$1 = position[1];
					const [r$1, g, b] = ImageColorUtils$2.getRGB(data, x$1, y$1, this.canvas.width);
					rgbArray.push([
						r$1,
						g,
						b
		...
		static isAdjust(oldVal, newVal, boundaryValue, type) {
			...
			if (!type || type === "rgb") {
				...
				const G$1 = G_1 - G_2;
				const B$1 = B_1 - B_2;
				distance = Math.sqrt((2 + rmean / 256) * Math.pow(R$1, 2) + 4 * Math.pow(G$1, 2) + (2 + (255 - rmean) / 256) * Math.pow(B$1, 2));
			} else if (type === "lab") {
		...
		static getAverage(data) {
			const total = data.reduce((x$1, y$1) => [
				x$1[0] + y$1[0],
				x$1[1] + y$1[1],
				x$1[2] + y$1[2]
			]);
		...
		static getMost(data) {
			const r$1 = majorityElement$1(data.map((item) => item[0]));
		...
		static getMedian(data) {
			const total0 = data.map((item) => item[0]).sort((x$1, y$1) => x$1 > y$1 ? 1 : -1);
			const total1 = data.map((item) => item[1]).sort((x$1, y$1) => x$1 > y$1 ? 1 : -1);
			const total2 = data.map((item) => item[2]).sort((x$1, y$1) => x$1 > y$1 ? 1 : -1);
			const total3 = data.map((item) => item[3]).sort((x$1, y$1) => x$1 > y$1 ? 1 : -1);
			const length = data.length;
			if (length % 2 === 0) {
				const r$1$1 = (total0[length / 2] + total0[length / 2 - 1]) / 2;
		...
		static getRGB(data, x$1, y$1, width) {
			const index = (width * (y$1 - 1) + x$1 - 1) * 4;
			const [r$1, g, b, a] = [
				data[index],
				data[index + 1],
				data[index + 2],
		...
		getArrayFromTopLine(leftTopPosition, rightBottomPosition) {
			...
			for (let x$1 = leftTopX; x$1 <= rightBottomX; x$1++) result.push([x$1, leftTopY]);
			return result;
		}
		getArrayFromRightLine(leftTopPosition, rightBottomPosition) {
			...
			for (let y$1 = leftTopY; y$1 <= rightBottomY; y$1++) result.push([rightBottomX, y$1]);
			return result;
		}
		getArrayFromBottomLine(leftTopPosition, rightBottomPosition) {
			...
			const rightBottomX = rightBottomPosition[0];
			const rightBottomY = rightBottomPosition[1];
			for (let x$1 = leftTopX; x$1 <= rightBottomX; x$1++) result.push([x$1, rightBottomY]);
			return result;
		}
		getArrayFromLeftLine(leftTopPosition, rightBottomPosition) {
			...
			const leftTopY = leftTopPosition[1];
			const rightBottomY = rightBottomPosition[1];
			for (let y$1 = leftTopY; y$1 <= rightBottomY; y$1++) result.push([leftTopX, y$1]);
			return result;
		}
		...
		adjust(leftTopPosition, rightBottomPosition) {
			...
			const x$1 = adjustLeftTopPosition[0];
			const y$1 = adjustLeftTopPosition[1];
			return {
				x: x$1,
				y: y$1,
				width: adjustWidth,
		...
		pickColors() {
			...
			const type = "lab";
			let lastColor;
			for (let x$1 = 1; x$1 < this.canvas.width; x$1 += ImageColorUtils$2.ParticleSize) for (let y$1 = 1; y$1 < this.canvas.height; y$1 += ImageColorUtils$2.ParticleSize) {
				const similarValues = Object.values(similarColorsMap);
				const rgba = ImageColorUtils$2.getRGB(this.imageData.data, x$1, y$1, this.canvas.width);
				lastColor = rgba;
			...
			values.sort((x$1, y$1) => x$1.length < y$1.length ? 1 : -1).filter((item) => item.length > Math.floor(this.imageData.data.length / (this.canvas.width * this.canvas.height) * 4)).forEach((item) => {
...
//#endregion
//#region ../../node_modules/.pnpm/image-color-utils@1.3.2/node_modules/image-color-utils/build/index.js
var require_build = __commonJS({ "../../node_modules/.pnpm/image-color-utils@1.3.2/node_modules/image-color-utils/build/index.js"(exports) {
	(function(l$1, r$1) {
		if (!l$1 || l$1.getElementById("livereloadscript")) return;
		r$1 = l$1.createElement("script");
		r$1.async = 1;
	...
	function rgb2value(rgb) {
		const [r$1, g, b] = rgb;
		const value = Math.max(r$1, g, b);
		return value;
	}
	function rgb2whiteness(rgb) {
		const [r$1, g, b] = rgb;
		const whiteness = Math.min(r$1, g, b);
		return whiteness;
	}
	function rgb2hue(rgb, fallbackhue = 0) {
		...
		if (delta) {
			const segment = value === r$1 ? (g - b) / delta : value === g ? (b - r$1) / delta : (r$1 - g) / delta;
			const shift = value === r$1 ? segment < 0 ? 6 : 0 : value === g ? 2 : 4;
	...
	function hex2rgb(hex) {
		const [, r$1, g, b, a, rr, gg, bb, aa] = hex.match(hexColorMatch) || [];
		if (rr !== undefined || r$1 !== undefined) {
			const red = rr !== undefined ? parseInt(rr, 16) : parseInt(r$1 + r$1, 16);
	...
	function rgb2hsl(rgb) {
		let r$1 = rgb[0] / 255, g = rgb[1] / 255, b = rgb[2] / 255, max = Math.max(r$1, g, b), min = Math.min(r$1, g, b), h, s, l$1 = (max + min) / 2;
		if (max === min) h = s = 0;
else {
			let d = max - min;
			s = l$1 > .5 ? d / (2 - max - min) : d / (max + min);
			switch (max) {
				case r$1:
					h = (g - b) / d + (g < b ? 6 : 0);
					break;
	...
	function rgb2hsv(rgb) {
		...
		g /= 255;
		b /= 255;
		let max = Math.max(r$1, g, b), min = Math.min(r$1, g, b);
		let h, s, v$1 = max;
		let d = max - min;
		s = max === 0 ? 0 : d / max;
		if (max === min) h = 0;
	...
	var ImageColorUtils = class ImageColorUtils {
		...
		pickColor(x$1, y$1) {
			return ImageColorUtils.getRGB(this.imageData.data, x$1, y$1, this.canvas.width);
		}
		pickLineColor({ leftTopPosition, rightBottomPosition, scopes }) {
			const data = this.imageData.data;
			const media = {};
			...
			for (const key in lineArrayCollection) {
				...
				for (const position of lineArray) {
					const x$1 = position[0];
					const y$1 = position[1];
					const [r$1, g, b] = ImageColorUtils.getRGB(data, x$1, y$1, this.canvas.width);
					rgbArray.push([
						r$1,
						g,
						b
		...
		static getMost(data) {
			const r$1 = majorityElement(data.map((item) => item[0]));
		...
		static getMedian(data) {
			...
			const length = data.length;
			if (length % 2 === 0) {
				const r$2 = (total0[length / 2] + total0[length / 2 - 1]) / 2;
				const g$1 = (total1[length / 2] + total1[length / 2 - 1]) / 2;
				const b$1 = (total2[length / 2] + total2[length / 2 - 1]) / 2;
				const a$1 = (total3[length / 2] + total3[length / 2 - 1]) / 2;
				return [
		...
		pickColors() {
			...
			const type = "lab";
			let lastColor;
			for (let x$1 = 1; x$1 < this.canvas.width; x$1 += ImageColorUtils.ParticleSize) for (let y$1 = 1; y$1 < this.canvas.height; y$1 += ImageColorUtils.ParticleSize) {
				const similarValues = Object.values(similarColorsMap);
				const rgba = ImageColorUtils.getRGB(this.imageData.data, x$1, y$1, this.canvas.width);
				lastColor = rgba;
				if (rgba[3] === 0) continue;
else if (!similarValues.length) similarColorsMap[similarValues.length] = [rgba];
	...
	exports.isDeepColorByHsl = isDeepColorByHsl;
	exports.isDeepColorByHsv = isDeepColorByHsv;
...
var require_react_production_min = __commonJS({ "../../node_modules/.pnpm/react@18.2.0/node_modules/react/cjs/react.production.min.js"(exports) {
...
var require_banner = __commonJS({ "src/banner.js"(exports) {
	...
	const image_color_utils_1 = require_build();
